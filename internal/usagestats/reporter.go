...
// sendFunc transmits an assembled usage report.
// The default is sendReport (HTTP to grafana.com); tests replace it with a fake.
type sendFunc func(ctx context.Context, seed *alloyseed.Seed, interval time.Time, metrics map[string]any) error
...
func StartReporter(ctx context.Context, logger *slog.Logger, seedDir string, tracker *Tracker) {
	...
	rep := &reporter{logger: logger, send: sendReport}
	go func() {
...
func (rep *reporter) reportUsage(ctx context.Context, interval time.Time, metrics map[string]any) error {
	...
	for backoff.Ongoing() {
		if err := rep.send(ctx, rep.seed, interval, metrics); err != nil {
